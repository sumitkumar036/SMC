...
const Home: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center py-4 md:py-8 px-2 overflow-x-hidden bg-transparent">
      ...
      {/* --- LAYER 1: Centered Side Ganesh Motifs (22% Width) --- */}
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden opacity-[0.2] dark:opacity-[0.3]">
        {/* Left Side Motif */}
        <div className="absolute top-1/2 left-[4%] md:left-[6%] -translate-y-1/2 w-[20%] max-w-[320px] h-auto transition-all duration-1000 ease-in-out">
            <img 
              src={ganeshBg} 
              alt="Ganesh Motif Left" 
              className="w-full h-auto object-contain filter saturate-[1.2] sepia-[0.2] dark:invert dark:brightness-[1.1] drop-shadow-[0_0_25px_rgba(255,215,0,0.15)]" 
            />
        </div>

        {/* Right Side Motif */}
        <div className="absolute top-1/2 right-[4%] md:right-[6%] -translate-y-1/2 w-[20%] max-w-[320px] h-auto transition-all duration-1000 ease-in-out">
            <img 
              src={ganeshBg} 
              alt="Ganesh Motif Right" 
              className="w-full h-auto object-contain filter saturate-[1.2] sepia-[0.2] dark:invert dark:brightness-[1.1] drop-shadow-[0_0_25px_rgba(255,215,0,0.15)]" 
            />
        </div>
      </div>

      {/* --- LAYER 2: Metallic Marquee (z-40) --- */}
      ...
      {/* --- LAYER 3: Falling Hearts Overlay (z-50) --- */}
